...
MEMORY_SCHEMA = {
    ...
    "description": (
        ...
        "and is injected into future turns. Two targets: 'user' (who the user is — "
        "name, role, preferences, communication style) and 'memory' (your notes — "
        "environment facts, project conventions, tool quirks, lessons learned). "
        "Actions: add, replace, and remove (old_text identifies the entry for "
        "replace/remove). Save proactively when the user corrects you or shares a "
        "preference; keep entries compact and durable. Don't store task progress or "
        "anything stale within a week — recall those with session_search."
    ),
    "parameters": {
        "type": "object",
        "properties": {
