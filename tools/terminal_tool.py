...
A terminal tool that executes shell commands on the local host. This lean
build ships a single backend — ``local`` (TERMINAL_ENV=local, the default).
...
def _get_scratch_dir() -> Path:
    """Return the scratch directory used for sandbox bookkeeping/cleanup.

    Honors ``TERMINAL_SCRATCH_DIR`` when set; otherwise falls back to the
    Hermes sandbox dir. (The lean build only runs the local backend, so this
    is just a cleanup root for any leftover ``hermes-*`` working dirs.)
    """
...
# Environment classes now live in tools/environments/. The lean build only
# ships the local backend.
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    raise ValueError(
        f"Unsupported terminal backend: {env_type!r}. This lean build only "
        "supports the 'local' backend — set TERMINAL_ENV=local (the default)."
    )
...
def check_terminal_requirements() -> bool:
    ...
    The lean build only supports the local backend; any other ``TERMINAL_ENV``
    ...
    try:
        ...
        if env_type == "local":
            return True
        logger.error(
            "Unsupported TERMINAL_ENV %r. This lean build only supports the "
            "'local' backend.",
            env_type,
        )
        return False
...
TERMINAL_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "background": {
                "type": "boolean",
                "description": "Run in the background, returning a session_id. Pair bounded jobs with notify_on_complete=true; long-lived processes (servers/daemons) stay silent. For short commands prefer foreground.",
                "default": False
            },
            "timeout": {
                "type": "integer",
                "description": f"Max seconds to wait (default 180, foreground max {FOREGROUND_MAX_TIMEOUT}). Returns instantly when the command finishes, so a high value costs nothing. For longer commands use background=true.",
                "minimum": 1
            },
            "workdir": {
                "type": "string",
            ...
            "pty": {
                "type": "boolean",
                "description": "Run in a pseudo-terminal for interactive CLI tools (Codex, Claude Code, REPLs). Local and SSH backends only.",
                "default": False
            },
            "notify_on_complete": {
                "type": "boolean",
                "description": "With background=true, notify you once when the process exits — use for almost every long-running bounded task. Mutually exclusive with watch_patterns.",
                "default": False
            },
            "watch_patterns": {
                "type": "array",
                "items": {"type": "string"},
                "description": "Strings to watch for in a background process's output (rate-limited). Use only for rare one-shot signals on long-lived processes (e.g. 'startup complete'); for end-of-run use notify_on_complete. Mutually exclusive with notify_on_complete."
            }
        },
        "required": ["command"]
    }
